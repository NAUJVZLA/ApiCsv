...
function pagination(
  ...
  // Previous
  if (currentPage > 1) {
    paginationHTML += `<li class="page-item"><a class="page-link"  data-page="${currentPage - 1
      }" href="#">Previous</a></li>`;
  }

  // numeros de los botones que van a aparecer
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  downloadButton.addEventListener("click", async (e: Event) => {
    ...
    await downloadCSV(csvData, "DatosFiltrados.csv");
  });
...
async function renderTableControls() {
  ...
  // haciendo que entren y sea seleccionado todo lo del documento . para poder acceder a las clases al hacer click
